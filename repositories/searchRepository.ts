...
export default class SearchRepository {
  ...
  static async searchBothByMunicipio(municipioTerm: string): Promise<{ chalets: any[], plans: any[] }> {
    try {
      // Ejecutamos el procedimiento almacenado
      const [results]: any = await db.query('CALL searchChaletsAndPlansByMunicipio(?)', [municipioTerm]);
